  ...
  try {
    ...
    if (req.body.loginType == 0) {
      ...
      if (!req.body.uniqueID) {
        return res
          .status(200)
          .json({ status: false, message: "uniqueID is required!!" });
      }
    ...
    if (req.body.loginType == 2) {
      if (!user || req.body.uniqueID != user.uniqueID) {
        return res
          .status(200)
          .json({ status: false, message: "Invalid uniqueID" });
      }
...
exports.addCoinByAdmin = async (req, res) => {
  try {
    ...
    if (req.body.coin < 0)
      return res
        .status(200)
        .json({ status: false, message: "Coin is not decrease!!" });
